...
import { 
  ...
  Pin, Filter, UserMinus, Zap, Star, Swords, Shield, ChevronRight, Shuffle
} from 'lucide-react';
...
export const RecordMatchModal: React.FC<RecordMatchModalProps> = ({
  ...
  const [onTimeSearchQuery, setOnTimeSearchQuery] = useState('');
  const [onTimeSelectedIds, setOnTimeSelectedIds] = useState<Set<string>>(new Set());
  const [showOnTimeMultiCheckboxes, setShowOnTimeMultiCheckboxes] = useState(false);
  
  // Match Sheet State - Defaults to Red Team & Green Team with Red/Green jerseys
  ...
  const { presentCount, absentCount, winCount, onTimeCount, totalGoals, totalOwnGoals, teamAScoredGoals, teamBScoredGoals, teamAOwnGoals, teamBOwnGoals, teamAGoals, teamBGoals } = useMemo(() => {
    let present = 0;
    let absent = 0;
    let win = 0;
    let ontime = 0;
    ...
    leagueData.players.forEach(p => {
      ...
      if (r?.goals) {
        ...
        goals += g;
        if (r.team === 'teamA') tAScoredGoals += g;
        if (r.team === 'teamB') tBScoredGoals += g;
      }
      if (r?.ownGoals) {
        const og = Number(r.ownGoals) || 0;
        ownGoals += og;
        if (r.team === 'teamA') tAOwnGoals += og;
        if (r.team === 'teamB') tBOwnGoals += og;
      }
    });

    // Own goals are credited to the opposing team's score!
    const tAGoals = tAScoredGoals + tBOwnGoals;
    const tBGoals = tBScoredGoals + tAOwnGoals;
    ...
    return {
      ...
      teamAScoredGoals: tAScoredGoals,
      teamBScoredGoals: tBScoredGoals,
      teamAOwnGoals: tAOwnGoals,
      teamBOwnGoals: tBOwnGoals,
      teamAGoals: tAGoals,
      teamBGoals: tBGoals,
    };
  }, [leagueData.players, records]);
  ...
  // Shuffle Teams: Randomly distribute currently attending players evenly between Team A and Team B
  const handleShuffleTeams = () => {
    const attendingPlayers = leagueData.players.filter(p => records[p.id]?.attendance);
    
    if (attendingPlayers.length === 0) {
      setLastAppliedFeedback({
        message: '⚠️ No attending players found to shuffle! Please mark attendee attendance first.',
        playerNames: [],
        timestamp: Date.now(),
      });
      return;
    ...
    if (attendingPlayers.length < 2) {
      setLastAppliedFeedback({
        message: '⚠️ Need at least 2 attending players to shuffle teams evenly.',
        playerNames: [],
        timestamp: Date.now(),
      });
      return;
    ...
    const shuffled = [...attendingPlayers];
    ...
    const teamAAssigned = shuffled.slice(0, half);
    const teamBAssigned = shuffled.slice(half);

    const teamASet = new Set(teamAAssigned.map(p => p.id));
    const teamBSet = new Set(teamBAssigned.map(p => p.id));
    ...
    setRecords(prev => {
      ...
      leagueData.players.forEach(p => {
        ...
        if (teamASet.has(p.id)) {
          let shouldWin = existing.win;
          if (matchWinner === 'teamA') shouldWin = true;
          else if (matchWinner === 'teamB' || matchWinner === 'draw') shouldWin = false;
        ...
        } else if (teamBSet.has(p.id)) {
          let shouldWin = existing.win;
          if (matchWinner === 'teamB') shouldWin = true;
          else if (matchWinner === 'teamA' || matchWinner === 'draw') shouldWin = false;
        ...
        } else if (!existing.attendance) {
          // Non-attending players have no team assignment
          next[p.id] = {
            ...existing,
            team: undefined,
            win: false,
    ...
    setLastAppliedFeedback({
      message: `🎲 Shuffled ${attendingPlayers.length} attendees evenly (${teamAAssigned.length} in 🔴 ${teamAName}, ${teamBAssigned.length} in 🟢 ${teamBName})`,
      playerNames: shuffled.map(p => formatPlayerName(p.name)),
      timestamp: Date.now(),
    });
  };

  // Shuffle only currently selected batch players evenly
  const handleShuffleSelected = () => {
    const targets = activeBatchPlayers.length > 0 ? activeBatchPlayers : (searchTerms.length > 0 ? filteredPlayers : []);
    if (targets.length < 2) {
      setLastAppliedFeedback({
        message: '⚠️ Select at least 2 players to shuffle.',
        playerNames: [],
        timestamp: Date.now(),
      });
      return;
    ...
    hasUserEditedRef.current = true;
    teamAAssigned.forEach(p => handleAssignPlayerTeam(p.id, 'teamA'));
    teamBAssigned.forEach(p => handleAssignPlayerTeam(p.id, 'teamB'));

    setLastAppliedFeedback({
      message: `🎲 Shuffled ${targets.length} selected players (${teamAAssigned.length} in 🔴 ${teamAName}, ${teamBAssigned.length} in 🟢 ${teamBName})`,
      playerNames: shuffled.map(p => formatPlayerName(p.name)),
      timestamp: Date.now(),
    });
  ...
  // Set On-Time (+1.0) for ALL attending players
  const handleSetOnTimeForAllAttending = (onTime: boolean) => {
    const attendingPlayers = leagueData.players.filter(p => records[p.id]?.attendance || records[p.id]?.team);
    if (attendingPlayers.length === 0) {
      setLastAppliedFeedback({
        message: '⚠️ No attending players found to mark on-time! Please mark attendance or assign players to teams first.',
        playerNames: [],
        timestamp: Date.now(),
      });
      return;
    ...
    setLastAppliedFeedback({
      message: onTime 
        ? `⏰ Marked all ${attendingPlayers.length} attending players as On-Time (+1.0)!`
        : `Cleared On-Time bonus from all ${attendingPlayers.length} attending players.`,
      playerNames: attendingPlayers.map(p => formatPlayerName(p.name)),
      timestamp: Date.now(),
    });
  };

  // Set On-Time (+1.0) for an entire team (Team A or Team B)
  const handleSetOnTimeForTeam = (team: 'teamA' | 'teamB', onTime: boolean) => {
    const teamPlayers = leagueData.players.filter(p => records[p.id]?.team === team);
    if (teamPlayers.length === 0) {
      setLastAppliedFeedback({
        message: `⚠️ No players assigned to ${team === 'teamA' ? teamAName : teamBName} yet.`,
        playerNames: [],
        timestamp: Date.now(),
      });
      return;
    ...
    hasUserEditedRef.current = true;
    const tName = team === 'teamA' ? teamAName : teamBName;
    setRecords(prev => {
      const updated = { ...prev };
    ...
    setLastAppliedFeedback({
      message: onTime 
        ? `⏰ Marked all ${teamPlayers.length} ${tName} players as On-Time (+1.0)!`
        : `Cleared On-Time bonus from ${tName}.`,
      playerNames: teamPlayers.map(p => formatPlayerName(p.name)),
      timestamp: Date.now(),
    });
  };

  // Batch toggle / apply On-Time to specific selected player IDs
  const handleBatchSetOnTimeForSelected = (playerIds: string[], onTime: boolean) => {
    ...
    setRecords(prev => {
      ...
      playerIds.forEach(pId => {
        ...
        updated[pId] = {
          ...existing,
          attendance: onTime ? true : existing.attendance,
          onTime: onTime,
        };
      });
      return updated;
    ...
    const names = playerIds.map(id => {
      const pl = leagueData.players.find(p => p.id === id);
      return pl ? formatPlayerName(pl.name) : id;
    });

    setLastAppliedFeedback({
      message: onTime 
        ? `⏰ Marked ${playerIds.length} selected player(s) as On-Time (+1.0): ${names.slice(0, 4).join(', ')}${names.length > 4 ? ` +${names.length - 4} more` : ''}`
        : `Removed On-Time bonus from ${playerIds.length} player(s)`,
      playerNames: names,
      timestamp: Date.now(),
    });

    setOnTimeSelectedIds(new Set());
  };

  // Toggle selection for On-Time multi-select tool
  const handleToggleOnTimePlayerSelection = (playerId: string) => {
    setOnTimeSelectedIds(prev => {
      const next = new Set(prev);
      if (next.has(playerId)) {
        next.delete(playerId);
      } else {
  ...
  // Select all attending players for On-Time multi-select
  const handleSelectAllOnTimeAttending = () => {
    const attending = leagueData.players.filter(p => records[p.id]?.attendance || records[p.id]?.team);
    setOnTimeSelectedIds(prev => {
      const next = new Set(prev);
      const allSelected = attending.length > 0 && attending.every(p => next.has(p.id));
      if (allSelected) {
        attending.forEach(p => next.delete(p.id));
      } else {
        attending.forEach(p => next.add(p.id));
      }
      return next;
    });
  };
  ...
  const handleProcessPasteRoster = (mode: 'match_sheet' | 'present_only' | 'all_bonus' | 'win' | 'ontime') => {
    ...
    lines.forEach(rawLine => {
      ...
      // Check for Team B header (Green Team, Blue Team, Non-Bibs, Rashu, Team 2, Team B)
      if (
        lineLower.includes('green') ||
        lineLower.includes('team green') ||
        lineLower.includes('greens') ||
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-gradient-to-br from-slate-900 via-slate-950 to-amber-950/30 p-4 rounded-2xl border-2 border-amber-500/60 shadow-xl space-y-4">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 border-b border-slate-800 pb-3.5">
              ...
              {/* Header Action Buttons: Shuffle Teams & Paste Roster */}
              <div className="flex items-center gap-2 flex-wrap">
                <button
                  type="button"
                  onClick={handleShuffleTeams}
                  className="px-3 py-1.5 rounded-xl text-xs font-black bg-gradient-to-r from-purple-600 via-indigo-600 to-amber-600 hover:from-purple-500 hover:to-amber-500 text-white shadow-md transition flex items-center space-x-1.5 cursor-pointer active:scale-95 border border-purple-400/40"
                  title="Automatically shuffle and assign all current attendees evenly between Team A and Team B"
                >
                  <Shuffle className="w-3.5 h-3.5" />
                  <span>🎲 Shuffle Teams {presentCount > 0 ? `(${presentCount})` : ''}</span>
                </button>
                ...
                <div className="md:col-span-4 space-y-1.5">
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      placeholder="Goals"
                      className="w-16 bg-slate-900 border border-emerald-500/50 focus:border-emerald-400 rounded-xl px-2 py-2 text-sm text-emerald-400 font-black text-center focus:outline-none"
                    ...
                    <input
                      ...
                      className="w-full bg-slate-900 border border-emerald-500/50 focus:border-emerald-400 rounded-xl px-3 py-2 text-sm text-white font-black text-right focus:outline-none"
                    />
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span>
                      {matchWinner === 'teamA' 
                        ...
                        : matchWinner === 'teamB'
                        ? `🟢 ${teamBName} (Green) won ${activeMatchTitle} on Friday ${matchDate}`
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
              ...
              <div className="bg-slate-950/90 rounded-xl border border-rose-500/50 p-3.5 space-y-3 shadow-md">
                <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                  ...
                  <span className="text-[11px] text-rose-400 font-black">
                    ⚽ {teamAGoals} Goals {teamBOwnGoals > 0 ? `(${teamAScoredGoals} + ${teamBOwnGoals} OG)` : ''}
                  </span>
                </div>
                    ...
                    teamAPlayers.map(player => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* Direct Interactive On-Time Toggle */}
                          <button
                            type="button"
                            onClick={(e) => {
                              e.stopPropagation();
                              handleTogglePlayerOnTime(player.id);
                            }}
                            className={`px-1.5 py-0.5 rounded text-[10px] font-black transition cursor-pointer flex items-center space-x-1 ${
                              r?.onTime
                                ? 'bg-amber-400 text-slate-950 ring-1 ring-amber-300 shadow-sm'
                                : 'bg-slate-900 text-slate-400 hover:text-amber-300 hover:bg-slate-800 border border-slate-700'
                            }`}
                            title={r?.onTime ? 'On-Time (+1.0 pt) - Click to unmark' : 'Click to mark On-Time (+1.0 pt)'}
                          >
                          ...
                          {r?.ownGoals ? <span className="text-red-400 font-black" title="Own Goal (counts for opponent)">🥅{r.ownGoals} OG</span> : null}
                          <button
                            type="button"
              ...
              {/* TEAM B ROSTER CARD (Green Team) */}
              <div className="bg-slate-950/90 rounded-xl border border-emerald-500/50 p-3.5 space-y-3 shadow-md">
                <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                  <div className="flex items-center space-x-2">
                    <span className="w-3 h-3 rounded-full bg-emerald-400"></span>
                    <h5 className="font-black text-emerald-300 text-xs sm:text-sm uppercase tracking-wide flex items-center gap-1.5">
                      <span>🟢 {teamBName}</span>
                      <span className="text-[11px] font-normal text-emerald-400/80">({teamBPlayers.length} Players)</span>
                    </h5>
                  </div>
                  <span className="text-[11px] text-emerald-400 font-black">
                    ⚽ {teamBGoals} Goals {teamAOwnGoals > 0 ? `(${teamBScoredGoals} + ${teamAOwnGoals} OG)` : ''}
                  </span>
                </div>
                  ...
                  {teamBPlayers.length === 0 ? (
                    ...
                    teamBPlayers.map(player => {
                      ...
                      return (
                        <div
                          key={player.id}
                          className="inline-flex items-center space-x-1.5 px-2 py-1 bg-emerald-950/80 border border-emerald-500/60 rounded-lg text-emerald-200 text-xs font-bold"
                        >
                          <span>🟢 {fName}</span>
                          {r?.win && <span title="Win (+1.0)">🏆</span>}
                          ...
                          <button
                            type="button"
                            onClick={() => handleAssignPlayerTeam(player.id, undefined)}
                            className="text-slate-400 hover:text-emerald-400 ml-1 p-0.5 rounded cursor-pointer"
                ...
                <div className="flex items-center space-x-2 pt-1">
                  <select
                    ...
                    defaultValue=""
                    className="w-full bg-slate-900 border border-emerald-700/60 text-xs text-emerald-200 rounded-lg px-2.5 py-1.5 font-bold focus:outline-none cursor-pointer"
                  >
                    <option value="" disabled>+ Add Player to 🟢 {teamBName} (Green)...</option>
                    {leagueData.players
            ...
            {/* ========================================================================= */}
            {/* ON-TIME ARRIVALS (+1.0 PTS) MULTI-PLAYER QUICK SELECTOR                   */}
            {/* ========================================================================= */}
            <div className="bg-slate-950/95 rounded-xl border border-amber-500/50 p-3.5 space-y-3 shadow-lg">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5 border-b border-slate-800 pb-3">
                <div className="flex items-center space-x-2.5">
                  <div className="p-2 bg-amber-500/20 text-amber-400 rounded-lg shrink-0 ring-1 ring-amber-500/30">
                    <Clock className="w-4 h-4" />
                  </div>
                  <div>
                    <h5 className="font-black text-amber-300 text-xs sm:text-sm uppercase tracking-wide flex items-center gap-2">
                      <span>⏰ On-Time Arrivals (+1.0 Pts)</span>
                      <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/40">
                        {onTimeCount} / {presentCount} Attending Marked
                      </span>
                    </h5>
                    <p className="text-[11px] text-slate-400">
                      Click any player below to toggle On-Time (+1.0 pt), or tick multi-players to apply at once.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center flex-wrap gap-1.5 w-full sm:w-auto justify-start sm:justify-end">
                  <button
                    type="button"
                    onClick={() => handleSetOnTimeForAllAttending(true)}
                    disabled={presentCount === 0}
                    className="px-2.5 py-1.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-black text-xs rounded-lg shadow transition flex items-center space-x-1 cursor-pointer active:scale-95 disabled:opacity-50"
                    title="Mark ALL attending players as On-Time (+1.0)"
                  >
                    <CheckCheck className="w-3.5 h-3.5" />
                    <span>⚡ All Attending ({presentCount})</span>
                  </button>

                  {teamAPlayers.length > 0 && (
                    <button
                      type="button"
                      onClick={() => handleSetOnTimeForTeam('teamA', true)}
                      className="px-2.5 py-1.5 bg-rose-700 hover:bg-rose-600 text-white font-bold text-xs rounded-lg shadow transition flex items-center space-x-1 cursor-pointer active:scale-95"
                      title={`Mark all ${teamAPlayers.length} players in ${teamAName} as On-Time (+1.0)`}
                    >
                      <span>🔴 All {teamAName} ({teamAPlayers.length})</span>
                    </button>
                  )}
                  ...
                  {teamBPlayers.length > 0 && (
                    <button
                      type="button"
                      onClick={() => handleSetOnTimeForTeam('teamB', true)}
                      className="px-2.5 py-1.5 bg-emerald-700 hover:bg-emerald-600 text-white font-bold text-xs rounded-lg shadow transition flex items-center space-x-1 cursor-pointer active:scale-95"
                      title={`Mark all ${teamBPlayers.length} players in ${teamBName} as On-Time (+1.0)`}
                    >
                      <span>🟢 All {teamBName} ({teamBPlayers.length})</span>
                    </button>
                  )}
                  ...
                  {onTimeCount > 0 && (
                    <button
                      type="button"
                      onClick={() => handleSetOnTimeForAllAttending(false)}
                      className="px-2 py-1.5 bg-slate-900 hover:bg-red-950 text-slate-400 hover:text-red-300 font-bold text-xs rounded-lg border border-slate-800 transition cursor-pointer"
                      title="Clear On-Time points from all players"
                    >
                      <span>Clear All ({onTimeCount})</span>
                    </button>
                  )}

                  <button
                    type="button"
                    onClick={() => setShowOnTimeMultiCheckboxes(prev => !prev)}
                    className={`px-2 py-1.5 rounded-lg text-xs font-bold border transition cursor-pointer ${
                      showOnTimeMultiCheckboxes
                        ? 'bg-amber-500/20 text-amber-300 border-amber-500/50 ring-1 ring-amber-500/30'
                        : 'bg-slate-900 text-slate-400 hover:text-slate-200 border-slate-800'
                    }`}
                    title="Toggle multi-select checkboxes mode"
                  >
                    <span>{showOnTimeMultiCheckboxes ? '☑ Multi-Select Active' : '☐ Multi-Select'}</span>
                  </button>
                </div>
              </div>

              {/* Multi-Select Floating Action Tray */}
              {showOnTimeMultiCheckboxes && (
                <div className="p-2.5 bg-amber-950/40 border border-amber-500/50 rounded-xl flex flex-wrap items-center justify-between gap-2 animate-in fade-in">
                  <div className="flex items-center space-x-2">
                    <button
                      type="button"
                      onClick={handleSelectAllOnTimeAttending}
                      className="px-2.5 py-1 rounded-lg text-xs font-bold bg-slate-900 border border-slate-700 text-amber-300 hover:bg-slate-800 transition cursor-pointer flex items-center space-x-1"
                    >
                      <CheckCheck className="w-3.5 h-3.5" />
                      <span>Select / Invert All Attendees ({presentCount})</span>
                    </button>
                    <span className="text-xs font-bold text-amber-200">
                      {onTimeSelectedIds.size} Selected
                    </span>
                  </div>

                  {onTimeSelectedIds.size > 0 && (
                    <div className="flex items-center space-x-2">
                      <button
                        type="button"
                        onClick={() => handleBatchSetOnTimeForSelected(Array.from(onTimeSelectedIds), true)}
                        className="px-3 py-1 bg-amber-500 hover:bg-amber-400 text-slate-950 font-black text-xs rounded-lg shadow transition cursor-pointer active:scale-95"
                      >
                        ✓ Mark ({onTimeSelectedIds.size}) On-Time (+1.0)
                      </button>
                      <button
                        type="button"
                        onClick={() => handleBatchSetOnTimeForSelected(Array.from(onTimeSelectedIds), false)}
                        className="px-2.5 py-1 bg-slate-900 hover:bg-red-950 text-slate-300 hover:text-red-300 text-xs font-bold rounded-lg border border-slate-700 transition cursor-pointer"
                      >
                        ✕ Remove On-Time
                      </button>
                      <button
                        type="button"
                        onClick={() => setOnTimeSelectedIds(new Set())}
              ...
              {/* Search filter for fast lookup */}
              <div className="relative">
                <input
                  type="text"
                  value={onTimeSearchQuery}
                  onChange={(e) => setOnTimeSearchQuery(e.target.value)}
                  placeholder="Filter attending players to toggle on-time..."
                  className="w-full bg-slate-900/80 border border-slate-800 rounded-lg pl-8 pr-8 py-1.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-amber-500/60"
                />
                <Search className="w-3.5 h-3.5 text-slate-500 absolute left-2.5 top-2 pointer-events-none" />
                {onTimeSearchQuery && (
                  <button
                    type="button"
                    onClick={() => setOnTimeSearchQuery('')}
              ...
              {/* Player interactive pills grouped by team */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
                {/* Team A Attending Players */}
                <div className="bg-slate-900/60 rounded-xl border border-rose-950/80 p-2.5 space-y-2">
                  <div className="flex items-center justify-between text-xs font-bold text-rose-300 pb-1.5 border-b border-slate-800">
                    <span className="flex items-center gap-1.5">
                      <span>🔴 {teamAName}</span>
                      <span className="text-[10px] text-rose-400/80 font-normal">({teamAPlayers.length} players)</span>
                    </span>
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-rose-950 text-rose-300 border border-rose-800/60 font-bold">
                      {teamAPlayers.filter(p => records[p.id]?.onTime).length} / {teamAPlayers.length} On-Time
                    </span>
                  </div>
                  <div className="flex flex-wrap gap-1.5 max-h-40 overflow-y-auto p-1">
                    {teamAPlayers.length === 0 ? (
                      <span className="text-[11px] text-slate-500 italic p-1">No players assigned to {teamAName}</span>
                    ) : (
                      teamAPlayers
                        .filter(p => !onTimeSearchQuery.trim() || p.name.toLowerCase().includes(onTimeSearchQuery.toLowerCase().trim()))
                        .map(player => {
                          const r = records[player.id];
                          const isSelected = onTimeSelectedIds.has(player.id);
                          const formatted = formatPlayerName(player.name);

                          if (showOnTimeMultiCheckboxes) {
                            return (
                              <label
                                key={player.id}
                                className={`inline-flex items-center space-x-1.5 px-2 py-1 rounded-lg text-xs font-bold border transition cursor-pointer ${
                                  isSelected
                              ...
                              >
                                <input
                                  type="checkbox"
                                  checked={isSelected}
                                  onChange={() => handleToggleOnTimePlayerSelection(player.id)}
                                  className="rounded bg-slate-950 border-slate-700 text-amber-500 focus:ring-0 w-3.5 h-3.5 cursor-pointer"
                                />
                                <span>{formatted}</span>
                                {r?.onTime && <span className="text-amber-400 font-black text-[10px]">⏰</span>}
                              </label>
                            );
                          }
                          ...
                          return (
                            <button
                              key={player.id}
                              type="button"
                              onClick={() => handleTogglePlayerOnTime(player.id)}
                              className={`inline-flex items-center space-x-1.5 px-2.5 py-1 rounded-lg text-xs font-bold transition cursor-pointer active:scale-95 ${
                                r?.onTime
                                  ? 'bg-amber-500 hover:bg-amber-400 text-slate-950 font-black shadow-sm ring-1 ring-amber-300'
                                  : 'bg-slate-900 hover:bg-slate-800 text-slate-300 hover:text-amber-200 border border-slate-700'
                              }`}
                              title={r?.onTime ? 'Marked On-Time (+1.0 pt) - Click to unmark' : 'Click to mark On-Time (+1.0 pt)'}
                            >
                              <Clock className={`w-3 h-3 ${r?.onTime ? 'text-slate-950' : 'text-slate-500'}`} />
                              <span>{formatted}</span>
                              <span className={`text-[10px] ${r?.onTime ? 'font-black' : 'text-slate-500 font-normal'}`}>
                                {r?.onTime ? '+1.0 ✓' : '+Time'}
                              </span>
                            </button>
                          );
                ...
                {/* Team B Attending Players */}
                <div className="bg-slate-900/60 rounded-xl border border-emerald-950/80 p-2.5 space-y-2">
                  <div className="flex items-center justify-between text-xs font-bold text-emerald-300 pb-1.5 border-b border-slate-800">
                    <span className="flex items-center gap-1.5">
                      <span>🟢 {teamBName}</span>
                      <span className="text-[10px] text-emerald-400/80 font-normal">({teamBPlayers.length} players)</span>
                    </span>
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-emerald-950 text-emerald-300 border border-emerald-800/60 font-bold">
                      {teamBPlayers.filter(p => records[p.id]?.onTime).length} / {teamBPlayers.length} On-Time
                    </span>
                  </div>
                  <div className="flex flex-wrap gap-1.5 max-h-40 overflow-y-auto p-1">
                    {teamBPlayers.length === 0 ? (
                      <span className="text-[11px] text-slate-500 italic p-1">No players assigned to {teamBName}</span>
                    ) : (
                      teamBPlayers
                        .filter(p => !onTimeSearchQuery.trim() || p.name.toLowerCase().includes(onTimeSearchQuery.toLowerCase().trim()))
                        .map(player => {
              ...
              {/* Other Attending / Unassigned Players (if any) */}
              {unassignedPlayers.length > 0 && (
                <div className="bg-slate-900/60 rounded-xl border border-amber-950/80 p-2.5 space-y-1.5">
                  <div className="text-xs font-bold text-amber-400 pb-1 border-b border-slate-800 flex items-center justify-between">
                    <span>⚪ Unassigned Attendees ({unassignedPlayers.length})</span>
                    <span className="text-[10px] text-amber-300/80">
                      {unassignedPlayers.filter(p => records[p.id]?.onTime).length} / {unassignedPlayers.length} On-Time
                    </span>
                  </div>
            ...
            {/* Unassigned Attending Players Alert Banner & Quick Auto-Balance */}
            {unassignedPlayers.length > 0 && (
              <div className="bg-slate-950/90 border border-amber-500/40 rounded-xl p-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5 shadow-md animate-in fade-in">
                <div className="flex items-start space-x-2.5">
                  ...
                  <div>
                    <span className="text-xs font-black text-amber-300 flex items-center gap-1.5">
                      <span>{unassignedPlayers.length} Attending Player{unassignedPlayers.length > 1 ? 's' : ''} not yet assigned to a team:</span>
                    </span>
                    <div className="flex flex-wrap gap-1 mt-1">
                      {unassignedPlayers.map(p => (
                        <span key={p.id} className="text-[11px] font-bold px-2 py-0.5 bg-slate-900 text-slate-200 rounded-md border border-slate-700">
                ...
                <button
                  type="button"
                  onClick={handleShuffleTeams}
                  className="px-3 py-1.5 rounded-lg text-xs font-black bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white shadow transition flex items-center space-x-1.5 shrink-0 cursor-pointer active:scale-95 border border-purple-400/40"
                  title="Randomly split all attending players into balanced Team A and Team B rosters"
                >
                  <Shuffle className="w-3.5 h-3.5" />
                  <span>🎲 Auto-Balance All Attendees</span>
                </button>
              </div>
            )}
          </div>
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-2.5 border-b border-slate-800 pb-2">
                  ...
                  <div className="flex items-center flex-wrap gap-1.5">
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 bg-emerald-700 hover:bg-emerald-600 text-white font-bold text-xs rounded-lg transition flex items-center space-x-1 cursor-pointer"
                    >
                      <span>🟢 Assign to {teamBName}</span>
                    </button>

                    {activeBatchPlayers.length >= 2 && (
                      <button
                        type="button"
                        onClick={handleShuffleSelected}
                        className="px-2.5 py-1 bg-purple-700 hover:bg-purple-600 text-white font-bold text-xs rounded-lg transition flex items-center space-x-1 cursor-pointer"
                        title="Shuffle only these selected players evenly between Team A and Team B"
                      >
                        <Shuffle className="w-3 h-3" />
                        <span>🎲 Shuffle Selected</span>
                      </button>
                    )}
                    ...
                    <button
                      ...
                      title="Apply Attendance (+0.5), Win (+1.0), and On-Time (+1.0) all at once"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => handleBatchSelectedAction('ontime_all')}
                      className="px-2.5 py-1 bg-amber-500 hover:bg-amber-400 text-slate-950 font-black text-xs rounded-lg shadow transition flex items-center space-x-1 cursor-pointer active:scale-95"
                      title="Mark selected players as On-Time (+1.0)"
                    >
                      <Clock className="w-3 h-3" />
                      <span>⏰ On-Time (+1.0)</span>
                    </button>

                    <button
                      type="button"
                      onClick={() => handleBatchSelectedAction('win_all')}
                      className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-lg shadow transition flex items-center space-x-1 cursor-pointer active:scale-95"
                      title="Mark selected players as Win (+1.0)"
                    >
                    ...
                    <button
                      ...
                      className="px-2 py-1 bg-slate-800 hover:bg-slate-700 text-white text-xs font-bold rounded-lg transition cursor-pointer"
                      title="Mark Attendance only (+0.5)"
                    >
                      <span>Att (+0.5)</span>
                    </button>
                    ...
                    <button
                      type="button"
                      onClick={() => handleBatchSelectedAction('ontime_none')}
                      className="px-2 py-1 bg-slate-900 hover:bg-amber-950 text-amber-400 hover:text-amber-200 text-xs font-bold rounded-lg border border-slate-800 transition cursor-pointer"
                      title="Remove On-Time bonus from selected players"
                    >
                      <span>- Time</span>
                    </button>
                  ...
                  {activeBatchPlayers.map(player => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {r?.team === 'teamB' && <span className="text-[10px] text-emerald-400 font-bold">(🟢 {teamBName})</span>}
                        <button
                          type="button"
                          onClick={() => {
                            setSelectedPlayerIds(prev => {
            ...
            <div className="space-y-1.5 pt-1">
              <div className="flex items-center justify-between text-[11px] text-slate-400">
                ...
                <span className="text-[10px] text-slate-400 font-bold">
                  <span className="text-rose-400 font-black">🔴 Red = {teamAName}</span> • <span className="text-emerald-400 font-black">🟢 Green = {teamBName}</span> • <span>⚪ Gray = Unassigned</span>
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-1.5 max-h-32 overflow-y-auto p-2 bg-slate-950/70 rounded-xl border border-slate-800">
                {filteredPlayers.map((player) => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg text-xs font-bold transition flex items-center space-x-1.5 cursor-pointer active:scale-95 ${
                        isSelected
                          ...
                          : isTeamB
                          ? 'bg-emerald-950/90 border border-emerald-500/70 text-emerald-200'
                          : isPresent
                    ...
                    >
                      <span className={`w-2 h-2 rounded-full ${isTeamA ? 'bg-rose-400' : isTeamB ? 'bg-emerald-400' : isPresent ? 'bg-emerald-400' : 'bg-slate-600'}`}></span>
                      <span>{fName}</span>
                    ...
                    >
                        ...
                        <div className="flex items-center space-x-2.5 truncate">
                          ...
                          <div className="truncate">
                            ...
                            <div className="flex items-center space-x-1.5 text-[10px] text-slate-400 font-medium">
                              ...
                              {r.team === 'teamB' && <span className="text-emerald-400 font-bold">• 🟢 {teamBName}</span>}
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="flex items-center space-x-1 shrink-0">
                        ...
                        <button
                          ...
                          title={`Assign to 🟢 ${teamBName} (Green)`}
                        >
