...
import { formatPlayerName, getDayOfWeekName, DEFAULT_TEAM_A_NAME, DEFAULT_TEAM_B_NAME } from '../../constants';
...
export const MatchDetailModal: React.FC<MatchDetailModalProps> = ({
  ...
  const teamAName = matchSheet?.teamAName || DEFAULT_TEAM_A_NAME;
  const teamBName = matchSheet?.teamBName || DEFAULT_TEAM_B_NAME;
  ...
  // Own goal rule: own goals scored by Team A players count towards Team B's score, and vice versa
  ...
  const handleShareOrCopyTeamSheet = async () => {
    ...
    if (winnerText) {
      ...
      text += `🛡️ *Matchup:* 🔴 ${teamAName} (Red) vs 🟢 ${teamBName} (Green)\n\n`;
    }

    text += `🔴 *${teamAName.toUpperCase()} (RED JERSEY)* [${teamAPlayers.length} Players • ${teamAGoalsCount} Goals${teamBOwnGoals > 0 ? ` (incl. ${teamBOwnGoals} OG)` : ''}]:\n`;
    if (teamAPlayers.length === 0) {
      ...
      teamAPlayers.forEach(({ player, record }, idx) => {
        ...
        if (record.ownGoals > 0) badges.push(`(OG ${record.ownGoals})`);
    ...
    text += `\n🟢 *${teamBName.toUpperCase()} (GREEN JERSEY)* [${teamBPlayers.length} Players • ${teamBGoalsCount} Goals${teamAOwnGoals > 0 ? ` (incl. ${teamAOwnGoals} OG)` : ''}]:\n`;
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `${match.title} Team Sheet • Friday Football League 2.0`,
          text: text,
        });
        return;
      } catch {
  ...
  return (
    <div className="match-detail-modal-wrapper fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-sm p-3 sm:p-4 animate-in fade-in duration-150">
      <div className={`match-detail-modal-box w-full max-w-4xl rounded-2xl shadow-2xl space-y-5 max-h-[92vh] overflow-y-auto transition-all ${
        ...
        {/* ========================================================================= */}
        {isPrintView && (
          <div className="no-print flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 pb-3 border-b border-slate-300">
            <div className="text-xs font-black text-slate-800 flex items-center gap-1.5">
              <span>📄 Official A4 Match Sheet Preview</span>
            </div>
            <div className="flex items-center space-x-2 flex-wrap gap-y-1">
              ...
              <button
                ...
                className="px-3 py-1.5 bg-rose-700 hover:bg-rose-600 text-white font-bold text-xs rounded-lg shadow transition flex items-center space-x-1.5 cursor-pointer active:scale-95"
                title="Export Clean PDF Match Sheet"
              >
              ...
              <button
                type="button"
                onClick={handleShareOrCopyTeamSheet}
                className="px-3 py-1.5 bg-emerald-700 hover:bg-emerald-600 text-white font-bold text-xs rounded-lg shadow transition flex items-center space-x-1.5 cursor-pointer active:scale-95"
                title="Share Lineup Text"
              >
                {copiedText ? <Check className="w-3.5 h-3.5 text-white" /> : <Share2 className="w-3.5 h-3.5" />}
          ...
          <div className="print-clean-container bg-white text-slate-900 p-4 sm:p-6 rounded-xl border border-slate-300 shadow-sm space-y-4">
            ...
            <div className="border-b-2 border-slate-900 pb-3 flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <FFLogo className="w-13 h-15 shrink-0 drop-shadow-sm" />
                <div>
              ...
              <div className="grid grid-cols-11 gap-2 items-center">
                ...
                <div className="col-span-3 flex flex-col items-center justify-center">
                  <div className="text-2xl sm:text-3xl font-black text-slate-950 tracking-widest">
                    ...
                    <span className="text-emerald-700">{teamBScore !== undefined ? teamBScore : teamBGoalsCount}</span>
                  </div>
                ...
                {/* Green Team Name & Badge */}
                <div className="col-span-4 bg-emerald-50 border border-emerald-300 rounded-lg p-2 flex items-center justify-between">
                  {winner === 'teamB' && (
                  ...
                  <div className="flex items-center space-x-2 text-right ml-auto">
                    <div>
                      <div className="font-black text-emerald-950 text-xs sm:text-sm uppercase tracking-wide">{teamBName}</div>
                      <div className="text-[10px] font-bold text-emerald-800">Green Jersey • {teamBPlayers.length} Players</div>
                    </div>
                    <span className="text-base">🟢</span>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-4 gap-2 text-center text-xs">
              ...
              <div className="bg-slate-50 border border-slate-300 rounded-lg p-2">
                <span className="text-[9px] uppercase font-bold text-slate-600 block">
                  {totalMatchOwnGoals > 0 ? `Total Goals (${totalMatchGoals} + ${totalMatchOwnGoals} OG)` : 'Goals Scored'}
                </span>
                <span className="text-sm font-black text-slate-900">
                  {totalMatchGoals + totalMatchOwnGoals} Goals
                </span>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <table className="w-full text-left border-collapse text-[11px]">
                    <thead>
                      <tr className="bg-rose-50 text-rose-950 border-b border-rose-200 text-[10px] uppercase font-black">
                        ...
                        <th className="py-1.5 px-1.5 text-center">OG</th>
                    ...
                    <tbody className="divide-y divide-slate-200">
                      {teamAPlayers.length === 0 ? (
                        <tr>
                          <td colSpan={8} className="py-4 text-center text-slate-500 italic text-[10px]">
                        ...
                        teamAPlayers.map(({ player, record }, idx) => {
                          ...
                          return (
                            <tr key={player.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'}>
                              ...
                              <td className="py-1 px-1.5 text-center font-bold text-red-600">{record.ownGoals > 0 ? `🥅${record.ownGoals}` : '—'}</td>
                    ...
                    <tfoot className="bg-rose-50 border-t border-rose-300 font-black text-[10px] text-rose-950">
                      <tr>
                        ...
                        <td className="py-1.5 px-1.5 text-center text-emerald-800" title={`Scored Goals: ${teamAScoredGoals}`}>{teamAScoredGoals}</td>
                        <td className="py-1.5 px-1.5 text-center text-red-600" title={`Own Goals conceded to Green: ${teamAOwnGoals}`}>{teamAOwnGoals > 0 ? teamAOwnGoals : '0'}</td>
                ...
                {/* GREEN TEAM SQUAD BOX */}
                <div className="border-2 border-emerald-700 rounded-xl overflow-hidden bg-white">
                  <div className="bg-emerald-700 text-white px-3 py-2 flex items-center justify-between">
                    <div className="flex items-center space-x-1.5 font-black text-xs uppercase tracking-wide">
                      <span>🟢</span>
                      <span>{teamBName} (Green Jersey)</span>
                    </div>
                    <span className="text-[10px] font-bold bg-white text-emerald-900 px-2 py-0.5 rounded-full">
                  ...
                  <table className="w-full text-left border-collapse text-[11px]">
                    <thead>
                      <tr className="bg-emerald-50 text-emerald-950 border-b border-emerald-200 text-[10px] uppercase font-black">
                        <th className="py-1.5 px-2 text-center w-6">#</th>
                        <th className="py-1.5 px-2">Player Name</th>
                        <th className="py-1.5 px-1.5 text-center">Att</th>
                        <th className="py-1.5 px-1.5 text-center">Win</th>
                    ...
                    <tfoot className="bg-emerald-50 border-t border-emerald-300 font-black text-[10px] text-emerald-950">
                      <tr>
                        <td colSpan={2} className="py-1.5 px-2">Green Team Totals:</td>
                        ...
                        <td className="py-1.5 px-1.5 text-center text-emerald-800" title={`Scored Goals: ${teamBScoredGoals}`}>{teamBScoredGoals}</td>
                        <td className="py-1.5 px-1.5 text-center text-red-600" title={`Own Goals conceded to Red: ${teamBOwnGoals}`}>{teamBOwnGoals > 0 ? teamBOwnGoals : '0'}</td>
            ...
            <div className="pt-4 border-t-2 border-slate-900 grid grid-cols-3 gap-4 text-[10px] text-slate-700">
              <div>
                ...
                {totalMatchOwnGoals > 0 && (
                  <div className="font-bold text-slate-900 mt-1 text-[9px]">
                    *Own Goal Rule: Own goals count towards opposing team score.
                  </div>
                )}
              </div>
          ...
          <>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-slate-800 pb-4 gap-3 no-print">
              ...
              <div className="flex items-center space-x-2 w-full sm:w-auto justify-end flex-wrap gap-y-1">
                {/* Share/Copy Team Sheet WhatsApp Text */}
                <button
                  type="button"
                  onClick={handleShareOrCopyTeamSheet}
                  ...
                  title="Share formatted lineup via WhatsApp or share sheet"
                >
                  {copiedText ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Share2 className="w-3.5 h-3.5" />}
                  <span>{copiedText ? 'Copied!' : 'Share Sheet'}</span>
                </button>
                ...
                <button
                  type="button"
                  onClick={() => onExportPDF(match)}
                  className="px-3 py-1.5 bg-slate-900 hover:bg-slate-800 text-rose-300 border border-rose-500/40 font-bold text-xs rounded-xl transition flex items-center space-x-1.5 cursor-pointer shadow active:scale-95"
                  title="Export official A4 PDF match sheet"
                >
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs rounded-xl transition flex items-center space-x-1.5 cursor-pointer shadow active:scale-95"
          ...
          <div className="grid grid-cols-1 md:grid-cols-11 gap-3 items-center text-center">
            ...
            <div className="md:col-span-3 flex flex-col items-center justify-center py-1">
              <div className="text-xl sm:text-2xl font-black text-amber-400 tracking-wider">
                ...
                <span className="text-emerald-400">{teamBScore !== undefined ? teamBScore : teamBGoalsCount}</span>
              </div>
            ...
            {/* Team B (Green Jersey) Badge & Name */}
            <div 
              onClick={() => setSelectedTeamTarget('teamB')}
              className={`md:col-span-4 p-3 rounded-xl flex items-center justify-between border transition cursor-pointer ${
                selectedTeamTarget === 'teamB'
                  ? 'ring-2 ring-emerald-400 border-emerald-500 bg-emerald-950/60 shadow-md shadow-emerald-950/50'
                  : 'bg-emerald-950/30 border-emerald-500/30 hover:border-emerald-500/60'
              }`}
              title="Click to select Green Team. Then click any player below to assign them here!"
            >
              {winner === 'teamB' && (
              ...
              <div className="flex items-center space-x-2.5 ml-auto text-right">
                <div>
                  <div className="flex items-center justify-end gap-1.5">
                    {selectedTeamTarget === 'teamB' && (
                      <span className="text-[9px] uppercase font-black px-1.5 py-0.2 bg-emerald-500 text-white rounded shadow animate-pulse">
                        Selected Target
                      </span>
                    )}
                    <span className="text-[10px] uppercase font-black px-1.5 py-0.2 bg-emerald-500/20 text-emerald-300 rounded border border-emerald-500/40">
                      Green
                    </span>
                    <span className="text-xs sm:text-sm font-black block text-emerald-300">
                      {teamBName}
                    </span>
                  </div>
                  <span className="text-[10px] font-semibold text-slate-400">
                ...
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-black text-sm border shadow-sm ${
                  selectedTeamTarget === 'teamB'
                    ? 'bg-emerald-500 text-white border-emerald-300 ring-2 ring-emerald-400'
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 text-center">
          ...
          <div className={`p-2.5 sm:p-3 rounded-xl border ${
            ...
            <span className={`text-xl font-black ${isPrintView ? 'text-emerald-800' : 'text-emerald-400'}`}>
              {totalMatchGoals + totalMatchOwnGoals}
            </span>
          </div>
        </div>
        ...
        {!isPrintView && viewTab === 'teams' && (
          <div className="space-y-4 animate-in fade-in duration-150">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center space-x-2 truncate">
                            ...
                            {record.ownGoals > 0 && (
                              <span className="px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-400 font-black text-[10px]" title="Own Goal (Counts for opposing team)">
                                OG {record.ownGoals}
                              </span>
                            )}
                            {record.onTime && (
              ...
              {/* TEAM B (GREEN JERSEY) LINEUP CARD */}
              <div className="bg-slate-950/90 rounded-2xl border border-emerald-500/50 p-4 space-y-3 shadow-lg">
                <div className="flex items-center justify-between border-b border-slate-800 pb-2.5">
                  <div className="flex items-center space-x-2">
                    <span className="w-3 h-3 rounded-full bg-emerald-400 shadow-sm"></span>
                    <h5 className="font-black text-emerald-300 text-sm sm:text-base uppercase tracking-wide flex items-center gap-1.5">
                      <span>🟢 {teamBName}</span>
                      <span className="text-[10px] uppercase font-black px-1.5 py-0.2 bg-emerald-500/20 text-emerald-300 rounded border border-emerald-500/40">Green Jersey</span>
                    </h5>
                  </div>
                  {winner === 'teamB' ? (
                    <span className="px-2 py-0.5 bg-emerald-600 text-white text-xs font-black rounded-lg shadow">
                      🏆 WIN (+1.0 PTS)
                    </span>
                  ) : (
                ...
                <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                  {teamBPlayers.length === 0 ? (
                    <div className="p-6 text-center text-xs text-slate-500 italic">
                      No players assigned to {teamBName} (Green Jersey).
                    </div>
                  ) : (
                    teamBPlayers.map(({ player, record }) => {
                      ...
                      return (
                        <div
                          key={player.id}
                          className="p-2.5 bg-emerald-950/20 border border-emerald-500/30 rounded-xl flex items-center justify-between text-xs hover:border-emerald-400/50 transition"
                        >
                          <div className="flex items-center space-x-2 truncate">
                            {player.avatar ? (
                              <img src={player.avatar} alt={player.name} className="w-6 h-6 rounded-full object-cover border border-emerald-500/40 shrink-0" />
                            ) : (
                              <span className="w-6 h-6 rounded-full bg-emerald-900/60 border border-emerald-700/60 text-emerald-300 font-bold flex items-center justify-center text-[10px] shrink-0">
                                {fName.charAt(0)}
                              </span>
                            )}
                            <span className="font-bold text-white truncate">{fName}</span>
                            ...
                            {record.ownGoals > 0 && (
                              <span className="px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-400 font-black text-[10px]" title="Own Goal (Counts for other team)">
                                OG {record.ownGoals}
                              </span>
                            )}
                            {record.onTime && (
                            ...
                            {record.win && (
                              <span className="text-[10px] text-emerald-400 font-bold" title="Match Win">🏆</span>
                            )}
                          </div>
                          <div className="flex items-center space-x-2 shrink-0">
                            ...
                            {onAssignPlayerTeam && (
                              <button
                                type="button"
                                onClick={() => onAssignPlayerTeam(match.id, player.id, undefined)}
                                className="text-slate-500 hover:text-emerald-400 p-0.5 rounded cursor-pointer transition"
            ...
            <div className="space-y-4 max-w-xl mx-auto">
              ...
              <div>
                <div className="flex justify-between text-xs font-bold mb-1">
                  ...
                  <span className="text-emerald-400">🟢 {teamBName}: {teamBPlayers.length} Players</span>
                </div>
              ...
              <div>
                <div className="flex justify-between text-xs font-bold mb-1">
                  ...
                  <span className="text-emerald-400">🟢 {teamBGoalsCount} Goals</span>
                </div>
                <div className="w-full h-3 bg-slate-900 rounded-full flex overflow-hidden border border-slate-800">
                  <div 
              ...
              <div>
                <div className="flex justify-between text-xs font-bold mb-1">
                  ...
                  <span className="text-emerald-400">🟢 {teamBOnTimeCount} ({teamBPlayers.length > 0 ? Math.round((teamBOnTimeCount / teamBPlayers.length) * 100) : 0}%)</span>
                </div>
                <div className="w-full h-3 bg-slate-900 rounded-full flex overflow-hidden border border-slate-800">
                  <div 
              ...
              <div>
                <div className="flex justify-between text-xs font-bold mb-1">
                  ...
                  <span className="text-emerald-400">🟢 +{teamBTotalPoints.toFixed(1)} pts</span>
                </div>
                <div className="w-full h-3 bg-slate-900 rounded-full flex overflow-hidden border border-slate-800">
                  <div 
                      ...
                      >
                        ...
                        <td className="py-2 px-2.5 text-center font-bold">
                          {r.team === 'teamA' ? (
                            ...
                            <span className="text-emerald-300 bg-emerald-950/60 px-1.5 py-0.5 rounded border border-emerald-500/40 font-black text-[10px] inline-flex items-center gap-1">
                              <span>🟢</span>
                              <span>{teamBName} (Green)</span>
                            </span>
                          ) : (
