    ...
    <div className="space-y-4 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5 bg-slate-950/80 p-3 rounded-xl border border-slate-800">
        <div className="flex items-center space-x-3">
          ...
          <div className="flex items-center space-x-2">
            <span className="w-3 h-3 rounded-full bg-sky-400 shadow-sm animate-pulse"></span>
            <span className="text-xs font-black text-sky-300">🔵 {teamBName} ({teamBPlayers.length})</span>
          </div>
        </div>
      ...
      <div className="relative w-full aspect-[16/10] sm:aspect-[16/9] md:aspect-[2/1] rounded-2xl overflow-hidden shadow-2xl border-4 border-emerald-900/60 select-none bg-emerald-900">
        ...
        {teamBPlayers.map(({ player, record }, idx) => {
          ...
          return (
            ...
            >
              {/* Player Jersey Disc */}
              <div className={`relative w-8 h-8 sm:w-10 sm:h-10 md:w-11 md:h-11 rounded-full bg-gradient-to-b from-sky-500 to-sky-700 text-white font-black flex items-center justify-center shadow-lg border-2 transition ${
                isSelected 
                  ? 'border-amber-300 ring-4 ring-amber-400/60 scale-110' 
                  : 'border-white/90 hover:border-amber-300'
              }`}>
              ...
              {/* Name Tag Pill */}
              <div className="mt-1 px-1.5 py-0.5 rounded bg-black/75 backdrop-blur-md border border-sky-500/40 text-[10px] sm:text-[11px] font-black text-sky-100 text-center leading-tight max-w-[80px] sm:max-w-[100px] truncate shadow">
                {fName}
              </div>
      ...
      {selectedPlayer && (
        <div className={`p-3.5 rounded-xl border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-lg animate-in slide-in-from-top-2 duration-150 ${
          selectedPlayer.team === 'teamA'
            ...
            : 'bg-sky-950/80 border-sky-500/60 text-sky-100'
        }`}>
          <div className="flex items-center space-x-3">
